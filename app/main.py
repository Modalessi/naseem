...
def copy_contents(src_dir: str, dest_dir: str):
    ...
    os.mkdir(dest_dir)
    contents = copy_content_helper(src_dir, dest_dir)

    for file in contents:
        print(f"[ COPIED ] {file.name}")


def copy_content_helper(path: str, dest_dir: str) -> list[os.DirEntry]:
    ...
    for entity in entites:
        ...
        if entity.is_file():
            shutil.copyfile(entity.path, os.path.join(dest_dir, entity_relative_path))
            contents.append(entity)
            continue

        os.mkdir(os.path.join("public", entity_relative_path))
        contents.extend(copy_content_helper(entity.path, dest_dir))
